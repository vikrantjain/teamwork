...
Then run the validator, spawn `teamwork:contract-auditor` with fresh context for
the judgement the validator cannot make, clear `friction.md`, commit, and
broadcast `RELOAD` carrying the team root's absolute path. **A member that is not
